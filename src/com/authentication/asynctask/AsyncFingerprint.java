...
import com.example.finger.TestFinger;
...
public class AsyncFingerprint extends Handler {
	...
	public static byte [] mModel=null;

	private FingerprintAPI fingerprint;

	public boolean isCancle = false;
	
	private static List<String> firStrings=new ArrayList<>();
	private static List<String> secStrings=new ArrayList<>();
	
	
	
	public static void setFinger (String fir,String sec) {
	}
	
	
	public static void setListFinger (List<String> fir,List<String> sec) {
		firStrings=fir;
		secStrings=sec;
	}
	
//	public AsyncFingerprint(String firfingerprintcode, String secfingerprintcode) {
//		this.firfingerprintcode = firfingerprintcode;
...
//		Log.e("model", model.length+"");
//		model.toString();
		byte[] base64=it.sauronsoftware.base64.Base64.encode(model);
		String string=null;
		try {
			string = new String(base64, "UTF-8");
//			Log.e("s", string);
//			Log.e("slength", string.length()+"");
...
//		String string = Base64.encodeToString(model, Base64.DEFAULT); 
		Configure.testFinger=string;
		Log.e("string", string);
		Log.e("stringlength", string.length()+"");
		RegisterFingerprintActivity.getFinger64Base(string);
			int count=DataSupport.count(TestFinger.class);
			if (count==0) {
				TestFinger mFinger=new TestFinger();
				mFinger.setMfinger(string);
				mFinger.save();
			}
			
//			else {
//				TestFinger findFirst = DataSupport.findFirst(TestFinger.class);
//			    ContentValues values=new ContentValues();
//			    values.put("mfinger", string);
//			    DataSupport.update(TestFinger.class, values, findFirst.getId());
//			}
			
		Log.e("decode.length", "model length=" + model.length);
...
//		mHandler.obtainMessage(SHOW_FINGER_IMAGE, image).sendToTarget();
		...
		boolean match =false;
		boolean isdoubledriver=false;
		    if (!TextUtils.isEmpty(firStrings.get(0))&&!TextUtils.isEmpty(secStrings.get(0))) {
				//说明是双加，
		    	isdoubledriver=true;
			}else {
				//单驾
		    ...
		    if (isdoubledriver) {
		    	   //第一驾驶员是没有按指纹的
		    	   if (TestFingerprintActivity.FirconfigFinger==0) {
		    		   for (String fircode : firStrings) {
		    			if (TextUtils.isEmpty(fircode)) {
		    				//跳出当次循环
							continue;
						}
		    			...
		    			byte[] byteIcon=null;
		    			try {
		    				byteIcon = fircode.getBytes("UTF8");
		    			} catch (UnsupportedEncodingException e) {
		    				// TODO Auto-generated catch block
		    				e.printStackTrace();
		    			}
		            	...
		            	if (match) {
		            		//第一驾驶员已经按下指纹
							TestFingerprintActivity.FirconfigFinger=1;
							return match;
						}
		    		   }
				     }else {
				    	   //第一驾驶员在按下指纹之后还继续按指纹对比
				    	   for (String fircode : firStrings) {
				    			if (TextUtils.isEmpty(fircode)) {
				    				//跳出当次循环
									continue;
		    	   ...
		    	   //第二驾驶员没有按过指纹
		    	   if (TestFingerprintActivity.SecconfigFinger==0) {
		    		   for (String seccode : secStrings) {
		   			...
		   			byte[] byteIcon=null;
	    			try {
	    				byteIcon = seccode.getBytes("UTF8");
	    			} catch (UnsupportedEncodingException e) {
	    				// TODO Auto-generated catch block
	    				e.printStackTrace();
	    			}
	    			...
	    			model=decode;
			            	...
			            	if (match) {
			            		//标志第二驾驶员已经按下了指纹
								TestFingerprintActivity.SecconfigFinger=1;
								return match;
						   }
		    		     }
					   }else {
						 //第二驾驶员在按下指纹之后还继续按指纹对比
						   for (String seccode : secStrings) {
				    			if (TextUtils.isEmpty(seccode)) {
				    				//跳出当次循环
									continue;
				 ...
				 //单驾驶员情况，第一驾驶员有信息
				 if (!TextUtils.isEmpty(firStrings.get(0))) {
					   for (String fircode : firStrings) {
			    			if (TextUtils.isEmpty(fircode)) {
			    				//跳出当次循环
								continue;
				...
				//单驾驶员情况，第二驾驶员有信息
				 if (!TextUtils.isEmpty(secStrings.get(0))) {
		    	    	//解码部分string 是服务端发来的信息
					  for (String seccode : secStrings) {
			    			if (TextUtils.isEmpty(seccode)) {
			    				//跳出当次循环
		    			...
		    			byte[] decode = Base64.decode(byteIcon);
		    			model=decode;
//		        	    byte[] byteIcon= Base64.decode(secfingerprintcode,Base64.DEFAULT);
//		        		model=byteIcon;
		        		int downChar = fingerprint.PSDownChar(2, model);
		        		if (downChar != 0x00) {
		        			return false;
