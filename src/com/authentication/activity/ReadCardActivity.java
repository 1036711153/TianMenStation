...
//				reader.read(12, 1, M1CardAPI.KEY_A, null);
	...
	private void initData() {
		...
		asyncParseSFZ.setOnReadSFZListener(new OnReadSFZListener() {
			...
			public void onReadFail(int result) {
				...
				if (result == ParseSFZAPI.Result.FIND_FAIL) {
					// ToastUtil
					// .showToast(ReadCardActivity.this, "未寻到卡,有返回数据    ");
					readSFZNotFind++;
		...
		reader.setOnReadAtPositionListener(new OnReadAtPositionListener() {
			...
			public void onReadAtPositionSuccess(String num, byte[][] data) {
				...
				cancleProgressDialog();
				rfidNum.setText("卡号：" + num + "\n数据：" + new String(data[0]));
				readRFIDSuccessTime++;
				refresh();
				if (!isStop) {
					// showProgressDialog("正在读取数据...");
			...
			public void onReadAtPositionFail(int comfirmationCode) {
				...
				if (comfirmationCode == M1CardAPI.Result.FIND_FAIL) {
					...
					// ToastUtil.showToast(ReadCardActivity.this, "认证密码失败");
					validateFail++;
				} else if (comfirmationCode == M1CardAPI.Result.READ_FAIL) {
					// ToastUtil.showToast(ReadCardActivity.this, "读卡失败");
					readFail++;
				}
				// ToastUtil.showToast(ReadCardActivity.this, "switchLength="
				// + reader.switchLength + "未寻到RFID！length="
	...
	private void refresh() {
		String result = "身份证总共：" + readSFZTime + "  成功：" + readSFZSuccessTime
				+ "  失败：" + readSFZFailTime + "未寻到卡有返回数据：" + readSFZNotFind
				+ "  超时：" + readSFZTimeout + "\n" + "RFID总共：" + readRFIDTime
				+ "  成功：" + readRFIDSuccessTime + "  失败：" + readRFIDFailTime
				+ "\n" + "RFID超时：" + timeoutTime + "  未寻到卡：" + notFindTime
				+ "  认证密码失败：" + validateFail + " 读卡失败：" + readFail;
