...
public class ICCardActivity extends BaseActivity implements OnClickListener{
	...
	private Button mBtnClean,mBtnGetRandom,mBtnSwitch,mBtnCycleGetRandom,mBtnClose,mBtnCleanCount;
	...
	private static String TAG = "ICCardActivity";
			...
			public void onItemSelected(AdapterView<?> parent, View view,
				...
				switch(position)
				{
					case 0:
						...
						break;
					case 1:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_DELETE_FILE));
						break;
					case 2:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_CREATE_MF));
						break;
					case 3:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_END_CREATE_MF));
						break;
					case 4:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_CREATE_DF));
						break;
					case 5:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_END_CREATE_DF));
						break;
					case 6:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_CREATE_BINARY));
						break;
					case 7:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_CHOOSE_BINARY));
						break;
					case 8:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_READ_BINARY));
						break;
					case 9:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_WRITE_BINARY));
						break;
					case 10:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_PROC_DIR));
						break;
					case 11:
						mEdSend.setText(DataUtils.toHexString(ICCardAPI.CMD_Get_Challenge));
						break;
				}
			}
			@Override
	...
	private void initData() {
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,
				android.R.layout.simple_spinner_item, getResources().getStringArray(R.array.ic_cmd_value));
	...
	public void onClick(View v) {
		switch(v.getId())
		{
			...
			case R.id.btn_cleanCount:
				count=0;
				mShowCount.setText(getResources().getString(R.string.ic_show_count) + count);
				break;
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
				case 0:
					...
					mRecvContent.setSelection(mRecvContent.getText().length(), mRecvContent.getText().length());
					break;
			}
		}
	};
	...
	class ListenThread extends Thread{
	    public void run(){
	    	isStop = false;
	    	mHandler.obtainMessage(0, "\r\n"+getResources().getString(R.string.ic_start_thread)).sendToTarget();
			while (!isStop) {
	...
	public static class LoopBufferIC implements LooperBuffer {
		...
		private byte[] ackCode ={0x3B,0x6C,0x00,0x02,(byte) 0x86,0x38,0x4f};
		...
		public synchronized byte[] getFullPacket() {
	        ...
	        if((tmpBuf=QueueList.poll())!=null)
	        {
	        	...
	        	if(tmpBuf.length > 14)
	        	{
	        		if(tmpBuf[5] ==ackCode[0]  && tmpBuf[6] == ackCode[1] && tmpBuf[7] == ackCode[2] 
	        				&& tmpBuf[8] == ackCode[3] && tmpBuf[11] == ackCode[4] && tmpBuf[12] == ackCode[5])
	        		{
	        			if((tmpBuf[10] & 0x9F) == tmpBuf[10]
	        					 || (tmpBuf[10] & 0x6F) == tmpBuf[10]) 
	        			{
	        				mHandler.obtainMessage(0, "\r\n"+ R.string.ic_init_success).sendToTarget();
	        			}
	        			else
	        				mHandler.obtainMessage(0, "\r\n" + R.string.ic_init_failure).sendToTarget();
	        			return null;
	        		}
	        	}
	        	...
	        	if( isCycle == true && tmpBuf.length > 7 && tmpBuf[tmpBuf.length - 3] == ackCodeByRandom[0] && tmpBuf[tmpBuf.length - 2] == ackCodeByRandom[1])
				{
	        		count++;
					mHandler.obtainMessage(0, "\r\n"+ R.string.ic_get_random_success + DataUtils.toHexString(tmpBuf)).sendToTarget();
					if(isCycle)
						pool.execute(task);
					return null;
				}
	...
	private static Runnable task = new Runnable() {
		@Override
		public void run() {
			byte[] CMD_Get_Challenge = {0x00,(byte) 0x84 ,0x00 ,0x00 ,0x04 };
			SerialPortManager.getInstance().write(api.makeProcData(CMD_Get_Challenge));
		}
	};
