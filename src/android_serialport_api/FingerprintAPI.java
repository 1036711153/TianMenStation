...
public class FingerprintAPI {
	...
	public synchronized byte[] PSUpChar(int bufferId) {
		...
		Log.e("PSUpChar", buffer.toString());
	...
	public synchronized int PSDownChar(int bufferId, byte[] model) {
		...
		sendCommand(command);
		int length = SerialPortManager.getInstance().read(buffer, 3000, 100);
		Log.e("PSDownChar", buffer.toString());
	...
	private synchronized int PS_ReadIndexTable(byte[] data, int pageId) {
		...
		sendCommand(command);
		int length = SerialPortManager.getInstance().read(buffer, 3000, 100);
		Log.e("PS_ReadIndexTable", buffer.toString());
...
//	 }
	...
	public synchronized byte[] PSUpImage() {
		...
		do {
			...
			Log.e("PSUpImage", buffer.toString());
			if(size == 0){
				break;
			}
			if (isFirst) {
				...
				if (size < startLength) {
					...
					Log.e("PSUpImage,bufferImage", bufferImage.toString());
					length += 12;
					SystemClock.sleep(5);
	...
	public synchronized int PSDownImage(byte[] image) {
		...
		sendCommand(command);
		int length = SerialPortManager.getInstance().read(buffer, 3000, 100);
		Log.e("PSDownImage", buffer.toString());
