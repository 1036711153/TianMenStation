...
public class M1CardAPI {
	...
	/**
	 * 函数说明: 读取指定块号存储的数据，长度一般为16字节 Reads the specified number stored data, length of 16 bytes
	 * 
	 * @param position 块号(S50 类型默认0-63)
	 * 
	 * @return 返回指定块号数据
	 */
	...
	private String makeCompletePassword(int keyType, String passwordHexStr) {
		...
		switch (keyType) {
		case KEY_A:
			completePasswordHexStr = passwordHexStr +"ff078069"+  DEFAULT_PASSWORD;
			break;
		case KEY_B:
			completePasswordHexStr = DEFAULT_PASSWORD + "ff078069" + completePasswordHexStr;
			break;
		default:
			break;
		}
